...
export default function CareerPortfolio({ careers }: { careers: Career[] }) {
      ...
      <span className="inline-flex gap-1.5 ml-2 align-middle">
        {!isBrand && (
          <a
            ...
            className="inline-flex items-center justify-center w-5 h-5 rounded bg-[#03C75A] text-[10px] font-black text-white hover:opacity-80 transition-opacity leading-none"
          >
            N
          </a>
        )}
        {hasImages && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              setModalImages(c.career_images);
            }}
            className="inline-flex items-center justify-center w-5 h-5 rounded bg-slate-200 text-[12px] hover:bg-slate-300 transition-colors cursor-pointer leading-none"
          >
            📸
          </button>
        )}
        {links.map((link: any) => {
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`inline-flex items-center justify-center w-5 h-5 rounded ${ytId ? "bg-[#FF0000]" : "bg-slate-800"} text-[10px] text-white hover:opacity-80 transition-opacity leading-none`}
            >
              {ytId ? "▶" : "L"}
            </a>
          );
  ...
  const renderList = (items: Career[], title: string) => (
    <div className="flex-1">
      <h3 className="text-xl font-black text-primary mb-10 pb-4 border-b-4 border-primary tracking-wide uppercase">
        {title}
      </h3>
              ...
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex flex-col relative flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="inline-flex items-center scale-90 origin-left">{renderIcons(c)}</div>
                  </div>
  ...
  return (
    <>
      <section className="py-24 px-[8%] border-b border-border bg-white overflow-visible">
        <h2 className="text-3xl font-black text-center tracking-[4px] uppercase text-primary mb-20">
          Career Portfolio
        </h2>
