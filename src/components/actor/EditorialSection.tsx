...
export default function EditorialSection({ editorials, awards }: { editorials: Editorial[], awards: Award[] }) {
  ...
  if (editorials.length === 0 && awards.length === 0) return null;
  ...
  return (
    <>
                      ...
                      <div className="group cursor-pointer" onClick={() => openModal(ed)}>
                        <div className="aspect-[3/4] rounded-lg overflow-hidden border border-border relative bg-muted">
                          ...
                          <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/30 transition-colors duration-300 flex items-center justify-center">
                            {count > 1 && <span className="opacity-0 group-hover:opacity-100 transition-opacity text-primary-foreground text-sm font-bold bg-foreground/50 px-2 py-0.5 rounded-full">+{count}</span>}
                          </div>
                          {thumb?.isVideo && <div className="absolute top-2 right-2 bg-foreground/60 text-primary-foreground text-[10px] px-1.5 py-0.5 rounded font-bold">▶</div>}
                        </div>
                        <div className="mt-4">
          ...
          <div className="max-w-[1200px] mx-auto pt-24 border-t-2 border-primary/30">
            <div className="flex flex-col md:flex-row gap-10 md:gap-20">
              ...
              <div className="flex-1">
                {awards.filter(a => a.show_on_profile).map((award) => (
                  <div key={award.id} className="group flex items-center justify-between py-5 border-b border-border/60 hover:bg-secondary/20 transition-colors px-2">
                    <div className="flex items-baseline gap-6">
                      <span className="text-sm font-black text-muted-foreground/60 tabular-nums w-12">{award.year_label}</span>
                      <span className="text-base font-extrabold text-foreground/90 group-hover:text-primary transition-colors">
                        {award.title}
                      </span>
                    </div>
                    
                    {/* 유튜브 링크 버튼 (빨간색 ▶ 버튼으로 수정) */}
                    {award.youtube_url && (
                      <button 
                        onClick={() => setActiveAwardVideo(getYouTubeId(award.youtube_url!))}
                        className="w-8 h-8 rounded bg-[#FF0000] flex items-center justify-center text-white text-[12px] hover:opacity-80 transition-all shadow-sm"
                        title="수상 장면 보기"
                      >
                        ▶
                      </button>
      ...
      {/* 시상식 영상 모달 */}
      {activeAwardVideo && (
        <div className="fixed inset-0 bg-foreground/95 z-[2500] flex items-center justify-center" onClick={() => setActiveAwardVideo(null)}>
          <div className="relative w-[90%] max-w-[1000px] aspect-video" onClick={e => e.stopPropagation()}>
            <button onClick={() => setActiveAwardVideo(null)} className="absolute -top-12 right-0 text-white text-3xl font-light hover:opacity-70">✕</button>
            <iframe
      ...
      {activeEditorial && currentMedia && (
        <div className="fixed inset-0 bg-foreground/95 z-[2000] flex items-center justify-center" onClick={() => setActiveEditorial(null)}>
          <button onClick={() => setActiveEditorial(null)} className="absolute top-6 right-8 text-white text-4xl font-light hover:opacity-70 z-10">✕</button>
          ...
          <div className="relative w-full max-w-[90vw] max-h-[90vh] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            {currentMedia.media_type === 'video' ? (
              ...
              <img src={currentMedia.media_url} alt="" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl" />
            )}
            {activeEditorial.editorial_media.length > 1 && (
              <>
                <button onClick={() => go(-1)} className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 text-white flex items-center justify-center text-xl hover:bg-white/40">‹</button>
                <button onClick={() => go(1)} className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 text-white flex items-center justify-center text-xl hover:bg-white/40">›</button>
              </>
            )}
          </div>
        </div>
