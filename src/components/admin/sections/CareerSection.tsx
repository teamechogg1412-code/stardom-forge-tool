      ...
      return (
        <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-xl bg-white">
          ...
          <Button 
            ...
            size="sm" 
            className="mt-4"
            onClick={() => setCareers([...careers, { category: categoryFilter, sub_category: '', year_label: '', title: '', description: '', role_image_url: '', links: [], images: [] }])}
          >
            + 첫 항목 추가하기
          </Button>
        </div>
    ...
    return (
      <DraggableList 
        ...
        onReorder={(reorderedFilteredItems: any) => {
          ...
          const newOrder = [...otherCategoryItems, ...reorderedFilteredItems.map(({originalIndex, ...rest}: any) => rest)];
          ...
          return (
            <div className="mb-6 p-6 border-2 border-slate-200 rounded-xl bg-white shadow-sm transition-all hover:border-primary/30">
              <div className="flex flex-wrap gap-3 items-start mb-5">
                ...
                <Button variant="ghost" size="sm" onClick={() => removeItem(i)} className="text-destructive hover:bg-destructive/10">✕</Button>
              </div>

              <div className="pl-4 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <Label className="text-[10px] font-black uppercase text-muted-foreground block mb-1">배역명 (Description)</Label>
                  ...
                  <div>
                    <Label className="text-[10px] font-black uppercase text-primary block mb-1">배역 프로필 이미지 URL (호버 미리보기용)</Label>
                    <Input placeholder="이미지 URL" value={c.role_image_url} onChange={e => updateItem(i, { role_image_url: e.target.value })} className="text-sm border-primary/20" />
                  </div>
                </div>

                <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 space-y-3">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider text-left">영상 및 외부 링크 관리</p>
                  {c.links.map((link: any, li: number) => (
                    <div key={li} className="flex gap-2">
                      <Input placeholder="라벨 (예: 예고편)" value={link.link_label} onChange={e => {
                        const newLinks = [...c.links]; newLinks[li].link_label = e.target.value; updateItem(i, { links: newLinks });
                      }} className="w-32 text-xs h-9 bg-white" />
                      <Input placeholder="URL" value={link.link_url} onChange={e => {
                        const newLinks = [...c.links]; newLinks[li].link_url = e.target.value; updateItem(i, { links: newLinks });
                      }} className="flex-1 text-xs h-9 bg-white" />
                      <Button variant="ghost" size="sm" onClick={() => {
                        const newLinks = c.links.filter((_: any, j: number) => j !== li); updateItem(i, { links: newLinks });
                      }}>✕</Button>
                    </div>
                  ))}
                  <Button variant="outline" size="sm" onClick={() => {
                    const newLinks = [...c.links, { link_url: '', link_label: '' }]; updateItem(i, { links: newLinks });
                  }} className="text-[10px] font-bold bg-white">+ 링크 추가</Button>
                </div>
              </div>
            </div>
          );
    ...
    <SectionWrapper title="커리어 포트폴리오 관리">
      <Tabs defaultValue="drama_film" className="w-full">
        <TabsList className="mb-6 bg-slate-100 p-1">
          <TabsTrigger value="drama_film" className="flex-1 font-bold text-xs data-[state=active]:bg-white data-[state=active]:text-primary">
            DRAMA & FILM ({careers.filter((c: any) => c.category === 'drama_film').length})
          </TabsTrigger>
          <TabsTrigger value="brand_editorial" className="flex-1 font-bold text-xs data-[state=active]:bg-white data-[state=active]:text-primary">
        ...
        <TabsContent value="drama_film" className="focus-visible:outline-none">
          ...
          <Button 
            ...
            onClick={() => setCareers([...careers, { category: 'drama_film', sub_category: '', year_label: '', title: '', description: '', role_image_url: '', links: [], images: [] }])}
          >
        ...
        <TabsContent value="brand_editorial" className="focus-visible:outline-none">
          ...
          <Button 
            variant="outline" 
            className="w-full mt-4 py-6 border-dashed border-2 font-black text-slate-400 hover:text-primary hover:bg-primary/5 transition-all" 
            onClick={() => setCareers([...careers, { category: 'brand_editorial', sub_category: '', year_label: '', title: '', description: '', role_image_url: '', links: [], images: [] }])}
          >
