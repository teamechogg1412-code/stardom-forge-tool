...
export default function EditorialSection({ editorials, setEditorials }: any) {
  ...
  const renderEditorialList = (categoryKey: string) => {
    ...
    const filtered = editorials
      ...
      .filter((item: any) => {
        if (categoryKey === 'other') {
          return !item.category || item.category === 'other' || !MAGAZINE_CATEGORIES.find(c => c.id === item.category);
        }
    ...
    return (
      <DraggableList items={filtered} droppableId={`eds-${categoryKey}`} onReorder={(reordered: any) => {
        // 순서 변경 시 원본 배열 업데이트 로직 (간단화를 위해 합침)
        const others = editorials.filter((_: any, idx: number) => !filtered.find((f: any) => f.originalIndex === idx));
        ...
        return (
          <div className="mb-6 p-5 border-2 border-slate-200 rounded-xl bg-white shadow-sm text-left">
            <div className="flex gap-3 items-center mb-4">
              ...
              <Input placeholder="연도" value={ed.year_label} onChange={e => updateEditorial(i, { year_label: e.target.value })} className="w-20 font-bold h-9" />
              <Input placeholder="매체 상세명 (예: W Korea 10월호)" value={ed.media_name} onChange={e => updateEditorial(i, { media_name: e.target.value })} className="flex-1 font-bold h-9" />
            ...
            <div className="pl-4 space-y-2">
              {ed.editorial_media?.map((m: any, mi: number) => (
                <div key={mi} className="flex gap-2 items-center bg-slate-50 p-2 rounded-lg border border-slate-100">
                  ...
                  <Input placeholder="URL" value={m.media_url} onChange={e => updateMedia(i, mi, { media_url: e.target.value })} className="flex-1 text-xs h-7" />
                  <Button variant="ghost" size="sm" onClick={() => {
                    const n = [...ed.editorial_media]; n.splice(mi, 1); updateEditorial(i, { editorial_media: n });
                  }} className="h-7 w-7 p-0 text-slate-400">✕</Button>
                </div>
              ))}
              <Button variant="outline" size="xs" onClick={() => addMedia(i)} className="text-[10px] font-bold">+ 사진 추가</Button>
            </div>
          </div>
        );
      }} />
        ...
        <div className="overflow-x-auto pb-2 mb-4">
          <TabsList className="flex w-max min-w-full bg-slate-100 p-1 h-auto">
            {MAGAZINE_CATEGORIES.map(cat => (
              ...
              >
                {cat.label} ({editorials.filter((e: any) => (cat.id === 'other' ? (!e.category || e.category === 'other') : e.category === cat.id)).length})
              </TabsTrigger>
            ))}
          </TabsList>
        </div>
        ...
        {MAGAZINE_CATEGORIES.map(cat => (
          <TabsContent key={cat.id} value={cat.id} className="focus-visible:outline-none">
            ...
            <Button 
              ...
              onClick={() => setEditorials([...editorials, { year_label: '', media_name: '', category: cat.id, editorial_media: [] }])}
            >
              + 새로운 {cat.label} 화보 추가
            </Button>
          </TabsContent>
